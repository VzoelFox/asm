...
# ===================================================================
# APPEND: Swap System & DateTime Integration
# ===================================================================

# Source modules
if [ -f "$(dirname "$0")/swap_system.sh" ]; then
    source "$(dirname "$0")/swap_system.sh"
fi

if [ -f "$(dirname "$0")/datetime.sh" ]; then
    source "$(dirname "$0")/datetime.sh"
fi

# Inject swap & datetime kernels into init_codegen
init_codegen_v2() {
    # Call original init_codegen
    init_codegen
    
    # Append swap system
    if type generate_swap_kernel &>/dev/null; then
        generate_swap_kernel
    fi
    
    # Append datetime
    if type generate_datetime_kernel &>/dev/null; then
        generate_datetime_kernel
    fi
}

# Override init_codegen if modules loaded
if type generate_swap_kernel &>/dev/null || type generate_datetime_kernel &>/dev/null; then
    alias init_codegen='init_codegen_v2'
fi
