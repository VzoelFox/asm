...
emit_output() {
    ...
    if [ ${#BSS_VARS[@]} -gt 0 ]; then
        # FIX: Exclude hardcoded vars to prevent duplication
        ...
        # Deduplicate using sort -u and filter out hardcoded vars
        local unique_vars=($(printf "%s\n" "${BSS_VARS[@]}" | sort -u))
        for var in "${unique_vars[@]}"; do
            # Check if var is in hardcoded list
            local is_hardcoded=0
            for hc in "${hardcoded_vars[@]}"; do
                if [ "$var" == "$hc" ]; then
                    is_hardcoded=1
                    break
                fi
            ...
            # Only emit if not hardcoded
            if [ $is_hardcoded -eq 0 ]; then
